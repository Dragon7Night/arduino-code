...
void intentarEnviarLogHistorial() {
  ...
  while (i < logLecturasCount) {
    ...
    if (enviarLecturaAFirebase(lec.tempDHT, lec.humDHT, lec.tempLM35, lec.timestamp)) {
      // Si se envió bien, eliminamos esa lectura del log corriendo el resto
      for (int j = i + 1; j < logLecturasCount; j++) {
        logLecturas[j - 1] = logLecturas[j];
      }
      logLecturasCount--;
    } else {
      Serial.println("[log] Fallo al enviar una lectura pendiente. Se reintentará más tarde.");
      break;  // Salimos para no entrar en bucle de errores
    }
  }
}
